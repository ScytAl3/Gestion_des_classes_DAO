<%-- 
    Document   : pageLesProfesseurs
    Created on : 11 mars 2019, 15:37:33
    Author     : Stagiaire
--%>

<%@page import="java.util.*"%> 
<%@page import="classMetier.Professeur"%> 
...
<html>
  <head>
	...
	<title>Page les professeurs JSP</title>
  ...
  <body>
	...
	<main class="text-center">
	  ...
	  <section style="background-color:#ffb3ff">
		<br>
		<h1>Bienvenue sur la page qui affiche les professeurs</h1>
		<hr class="mb-4">
		<h3>Information liées aux professeurs</h3>
		<br>
		
		<div class="row mb-2">
		<!--*****************************************************************-->
		  <article class="col-md-6">
			<p><strong>nombre de professeurs : </strong>${ requestScope.nombre }</p>
			...
			<table>
			  ...
			  <c:forEach var="prof" begin="0" items="${requestScope.lesProfesseurs}">
			  <tr>			
				<td>${prof.id}</td>
				<td>${prof.nom}</td>
				<td>${prof.prenom}</td>
		...
		<!--*****************************************************************-->	
		  <article class="col-md-6">
			...
			<form class="container" method="post">
			  <label for="lastName"><strong>Professeur à ajouter</strong></label>
			  <!--Champ pour la saisie du nom-->
			  <div class="mb-3">
				<input class="form-input mr-sm-2" name="lastName" id="lastName" type="text" 
					   placeholder="Saisir le nom du professeur..."
			  ...
			  <div class="mb-3">
				<input class="form-input mr-sm-2" name="firstName" id="firstName" type="text" 
					   placeholder="Saisir le prénom du professeur..."
			  ...
			  <div class="mb-3">				
				<p><strong>Choix de la matière :</strong></p>
				  <select name="choix-matiere" id="choix-matiere">
					<option value="0">--Sélectionnez la matière--</option>
					<c:forEach var="matiere" begin="0" items="${requestScope.lesMatieres}">
					  <option value="${matiere.id}"
							  <c:if test="${matiere.id eq matiereID}">selected="selected"</c:if>
							  >${matiere.nom}</option>
					</c:forEach>					
				  </select>				
			  </div>
			  <!--Recuperation du message d erreur si mauvaise saisie-->
			  <span class="input-error mb-3">${erreur['choix-matiere']}</span>
